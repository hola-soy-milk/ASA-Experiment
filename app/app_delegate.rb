class AppDelegate
  ...
  def start_experiment
    ...
    @test_count = 0
    @doing_part_2 = false
    @van_hoorden_sounds = NSFileManager.defaultManager.contentsOfDirectoryAtPath(NSBundle.mainBundle.resourcePath, error:nil).select {|file| file =~ /vn/}
    @van_hoorden_sounds = @van_hoorden_sounds + @van_hoorden_sounds.dup
    @van_hoorden_sounds.map! {|file| Tone.new(file)}
    NSLog "#{@van_hoorden_sounds.size}"
    @miller_and_heise_sounds = NSFileManager.defaultManager.contentsOfDirectoryAtPath(NSBundle.mainBundle.resourcePath, error:nil).select {|file| file =~ /mh/}
